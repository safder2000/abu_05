...
// This function will be passed the readline interface from index.js
let rlInterface;
const setReadlineInterface = (rl) => {
  rlInterface = rl;
};
...
const question = (query) =>
  new Promise((resolve) => {
    if (!rlInterface) {
      console.error("Error: Readline interface not set");
      resolve("");
      return;
    }
...
async function spawnBots(config) {
  try {
    const numBotsInput = await question(
      'Enter the number of bots to spawn (0 or "all" for all farms): ',
    );
...
module.exports = { spawnBots, setReadlineInterface };
