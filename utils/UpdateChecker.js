...
class UpdateChecker {
  ...
  startPeriodicChecks() {
    if (!this.isGitInstalled() || !this.isGitRepo) {
      console.log('Update checker disabled: Git not installed or not a Git repository');
      return;
    }
    ...
    console.log(`Update checker started. Will check for updates every ${this.checkInterval / 60000} minutes`);
  }
  ...
  checkForUpdates() {
    ...
    try {
      ...
      const behindCount = parseInt(
        execSync(`git rev-list --count HEAD..origin/${branch}`).toString().trim()
      );

      if (behindCount > 0) {
        const message = `Updates available: ${behindCount} new commit(s). Run update.bat to update.`;
